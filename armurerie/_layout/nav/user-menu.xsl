...
<xsl:template name="loginLogoutLink">
	...
	<xsl:variable name="login-status" select="document('../../login-status.php')" />   
            ...
            <div class="user-mod" style="text-align: right;">
                ...
                <xsl:choose>                    
                    <xsl:when test="string-length($txtPinClass) = 0">
                        ...
                        <br />
                        <a id="logoutLink" href="index.php?logout=1"><xsl:value-of select="$loc/strs/login/str[@id='armory.login.logout']" /></a>
                    </xsl:when>
                    <xsl:otherwise>
						<span class="loggedInAs"><xsl:value-of select="$loc/strs/login/str[@id='armory.login.logged']"/></span>                        
                        <br />
                                                
                        <a href="character-sheet.php?{$pinned/@url}" class="userName iconClassId{$pinned/@classId} staticTip" onmouseover="setTipText('{$txtPinClass}')">
...
<xsl:template name="userMenu">	
    ...
    <xsl:variable name="bookmark" select="document('../../bookmarks.php')/page/characters" />
		  ...
		  <xsl:for-each select="$profile[@selected &lt; 5]">
				...
				<div class="menuItem charlist">
                    ...
                    <div class="menuItemMeta">
                        <a href="/vault/character-calendar.php?{@url}" class="user-alerts staticTip" style="display: none;" onmouseover="setTipText('{$loc/strs/login/str[@id='armory.login.calendar.pending']}')">0</a>

                        <a href="character-sheet.php?{@url}" class="charName js-character-name"><xsl:value-of select="@name" /></a> -
                        <span class="js-character-realm"><xsl:value-of select="@realm" /></span>

                        <p>
                            <a href="character-achievements.php?{@url}" class="character-achievement staticTip" onmouseover="setTipText('{$loc/strs/login/str[@id='armory.login.achievements']}');"><xsl:value-of select="@achPoints" /></a>
