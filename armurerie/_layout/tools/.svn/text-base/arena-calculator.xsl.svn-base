...
<blockquote><b class="iarenateams"><h4><a href="battlegroups.php"><xsl:value-of select="$loc/strs/arena/str[@id='arena']"/></a></h4><h3><xsl:value-of select="$loc/strs/arenaCalc/str[@id='title']"/></h3></b></blockquote>
   ...
   function alertContents() {
      if (http_request.readyState == 4) {
         if (http_request.status == 200) {
			...
			if (guildName)
				document.getElementById('replacePinGuild').innerHTML = '&lt;<a href = "guild-info.php?'+ guildUrl +'">'+ guildName +'</a>&gt;';
			...
			for (var i = 0; i < ratingsArrayTemp.length; i++) {
				...
				document.getElementById('href'+ratingsArray[i][0]).href = "team-info.php?"+ ratingsArrayTemp[i].getAttribute("url");
	...
	<tr>
		<td colspan="2">
			<span>
				...
				<a href="search.php?fl%5Bsource%5D=pvpAlliance&amp;fl%5Bpvp%5D=arena{../@key}&amp;fl%5Btype%5D=all&amp;searchType=items">
