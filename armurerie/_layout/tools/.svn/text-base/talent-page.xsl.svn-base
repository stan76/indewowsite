					...
					<xsl:for-each select="$talentList/page/talentTrees/tree"> <!-- Loop through trees -->
						...
						<div id="{@key}_treeContainer_{$uniqueId}" class="talentTreeContainer" style="display: none">
							<div id="{@key}_tree_{$uniqueId}" class="talentTree" style="margin-right: 3px; background-image: url('images/talents/bg/small/{@bgImage}.jpg')">
								...
								<div class="talentTreeInfo" style="background: url(http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png) 0 0 no-repeat;">
...
<xsl:template name="oneTier">
	...
	<div class="tier">	
		<xsl:for-each select="$currTree/talent[@tier = $currTier]">
			<div id="{@key}_iconHolder_{$uniqueId}" class="talent staticTip col{@column}" style="background-image:url('http://eu.wowarmory.com/wow-icons/_images/_talents31x31/{@icon}.jpg');">
				<xsl:if test="not($currTier = 0)">
					<xsl:attribute name="style">background-image:url('http://eu.wowarmory.com/wow-icons/_images/_talents31x31/grey/<xsl:value-of select="@icon"/>.jpg');</xsl:attribute>
				</xsl:if>
