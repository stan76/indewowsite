...
<xsl:template name="charContent">
    ...
    <div class="activity_bg">
		...
		<xsl:if test="document(concat('../../character-model.php?rhtml=false&amp;',character/@charUrl))/page/character/@owned = '1'">
			...
			<a href="character-feed.atom?{character/@charUrl}" onclick="location.href='/custom-rss.php?{character/@charUrl}'; return false;" class="share_rss_icon"></a></div>
        ...
        <div class="activity_body_rpt"><div class="activity_body_top"><div class="activity_body_btm"><div class="activity_body_int">
            <xsl:variable name="feeddata" select="document(concat('../../character-feed-data.php?',character/@charUrl,'&amp;full=true&amp;loc=',$lang))" />
    ...
    <xsl:if test="position() mod 2 = 0"><xsl:attribute name="class">feed_entry zebra</xsl:attribute></xsl:if>     
        <table><tr><td class="td_icon">
            <div class="feed_icon"><a class="event_{@type}">
                <xsl:if test="@icon"><xsl:attribute name="style">background-image:url(http://eu.wowarmory.com/wow-icons/_images/21x21/<xsl:value-of select="@icon"/>.png)</xsl:attribute>
                ...
                <xsl:attribute name="href">item-info.php?i=<xsl:value-of select="@id"/> </xsl:attribute>
                </xsl:if>
...
<div class="custom_rss_container">
        ...
        <h2><xsl:value-of select="$loc/strs/custom-rss/str[@id='feedoptions']"/></h2>
                ...
                <!-- Dummy loc parameter guarantees language-specific client-side caching  -->
                    <td><xsl:for-each select="document(concat('/data/achievementStrings.php?loc=',$lang))/page/rootCategories/category">
            ...
            <h3><xsl:value-of select="$loc/strs/custom-rss/str[@id='add_chars']"/>
                ...
                <xsl:choose>
                    <xsl:when test="string-length($username) != 0">
                        <xsl:variable name="pinned_char" select="document('../../character-select.php?sel=2')/page/characters/character[@selected=1]" />
                        <xsl:choose>
