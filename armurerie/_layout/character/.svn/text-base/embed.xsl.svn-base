...
<xsl:choose>
    ...
    <xsl:attribute name="class">embed_bg <xsl:if test="character/@factionId='0'">embed_Alliance</xsl:if><xsl:if test="character/@factionId='1'">embed_Horde</xsl:if><xsl:if test="character/@factionId='2'">embed_Propass</xsl:if></xsl:attribute>
        <div class="points">
            <a href="character-achievements.php?{$c/@charUrl}" onclick="window.open(this.href);return false;"><xsl:value-of select="$points" /></a>
        </div>
        <div id="forumLinks">
        ...
        <div id="charHeaderTxt_Light">
            <div class="charNameHeader"><a class="charLink" href="character-sheet.php?{$c/@charUrl}" onclick="window.open(this.href);return false;"><xsl:value-of select="/page/characterInfo/character/@name" /></a></div>
            <xsl:if test="not($c/@guildName = '')">
                <div class="charGuildName"><a href="guild-info.php?{$c/@guildUrl}" onclick="window.open(this.href);return false;"><xsl:value-of select="$c/@guildName" /></a></div>
            </xsl:if>
    ...
    <a href="character-sheet.php?{$c/@charUrl}" onclick="window.open(this.href);return false;"><xsl:value-of select="$loc/strs/embed/str[@id='armory.embed.armorylink']"/></a>
    </div>
    </div>
    </xsl:otherwise>
</xsl:choose>
