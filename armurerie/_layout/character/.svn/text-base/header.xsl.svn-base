...
<xsl:template name="newCharHeader">
	...
	<div class="profile_header header_Horde">	
        ...
        <!-- achievement points -->
			<div class="points">
				<a href="character-achievements.php?{$c/@charUrl}"><xsl:value-of select="$points" /></a>
			</div>
		...
		<div id="profileRight" class="profile-right">				
			<xsl:choose>
				<xsl:when test="$IS_LOGGED_IN = 'true'">
					<xsl:variable name="bm" select = "document('../../bookmarks.php')/page/characters"/>
		...
		<div class="profile-achieve">			
			<a href="character-sheet.php?{/page/characterInfo/character/@charUrl}" class="staticTip">
            ...
            <!-- Auto-discovery of character feed -->	
				<link href="atom/character.php?{$c/@charUrl}" title="WoW Feed" rel="alternate" type="application/atom+xml"/>
		...
		<div id="charHeaderTxt_Light">
			<xsl:if test="not($c/@guildName = '')">
				<div class="charGuildName"><a href="guild-info.php?{$c/@guildUrl}"><xsl:value-of select="$c/@guildName" /></a></div>
			</xsl:if>
			<span class="prefix">
