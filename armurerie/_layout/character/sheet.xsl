    ...
    <!-- New right column -->
          <div class="char-profile_stats"><div class="char-profile_stats_int">
            ...
            <div class="spec">
                    ...
                    <a id="replaceTalentSpecText" href="character-talents.php?{$cUrl}&amp;group=1">
                        <xsl:if test="$groupOne[@active='1']">
                            <xsl:attribute name="class">active_spec</xsl:attribute>
                        </xsl:if>
                        <span class="talent_icon"><img src="http://eu.wowarmory.com/wow-icons/_images/43x43/{characterTab/talentSpecs/talentSpec[@group='1']/@icon}.png" /></span>
            ...
            <xsl:choose>
                <xsl:when test="$groupTwo">
                    <a class="right_spec" id="replaceTalentSpecText2" href="character-talents.php?{$cUrl}&amp;group=2">
                        <xsl:if test="$groupTwo[@active='1']">
                            <xsl:attribute name="class">right_spec active_spec</xsl:attribute>
                        </xsl:if>								
                        <span class="talent_icon"><img src="http://eu.wowarmory.com/wow-icons/_images/43x43/{characterTab/talentSpecs/talentSpec[@group='2']/@icon}.png" /></span>
          ...
          <!-- Character mini-Feed  -->
            ...
            <div class="char_feed_title">
            	...
            	<xsl:if test="document(concat('../../character-model.php?rhtml=false&amp;',character/@charUrl))/page/character/@owned = '1'">
				...
				<a href="character-feed.atom?{character/@charUrl}" onclick="location.href='custom-rss.php?{character/@charUrl}'; return false;" class="share_rss_icon"></a>
				</div>            	
                <xsl:value-of select="$loc/strs/custom-rss/str[@id='recent_act']" />
            </div>
            <xsl:variable name="feed_doc" select="document(concat('../../character-feed-data.php?',character/@charUrl,'&amp;loc=',$lang))"/>
        ...
        <xsl:for-each select="$feed_doc/feed/event[position()&lt;=6]">
            <div class="feed_entry"><xsl:if test="position() = last()"><xsl:attribute name="class">feed_entry last_entry</xsl:attribute></xsl:if>
                <table><tr><td class="td_icon">
                    <div class="feed_icon"><a class="event_{@type}">
                        ...
                        <xsl:attribute name="href">http://fr.wowhead.com/item=<xsl:value-of select="@id"/> </xsl:attribute>
                        </xsl:if>
            ...
            <a class="more_char_act" href="character-feed.php?{character/@charUrl}"><xsl:value-of select="$loc/strs/custom-rss/str[@id='viewfull_feed']"/></a>
          </div>
...
<!--end new right column -->
    ...
    <div class="embedLink" style="display:none"> &lt;iframe src="character-model-embed.php?<xsl:value-of select="character/@charUrl"/>&amp;rhtml=true" scrolling="no" height="588" width="321" frameborder="0"&gt;&lt;/iframe&gt; </div>
    <!--
...
<xsl:param name="cUrl"/>
			...
			<div class="achRight">
				...
				<a class="achPointsLink" href="character-achievements.php?{$cUrl}">
...
<script type="text/javascript" src="http://static.wowhead.com/widgets/power.js"></script>
    ...
    <xsl:for-each select="characterTab/items/item">
    	<tr>
        	<td class="glist_icon">
            <a id="i={@id}&amp;{$c_Url}&amp;s={@slot}" href="http://fr.wowhead.com/item={@id}" class="staticTip itemToolTip">		
                <img class="stats_rarity{@rarity}" src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png"/>
            </a>
            </td>
            <td class="glist_name" >
                <a id="i={@id}&amp;{$c_Url}&amp;s={@slot}" href="http://fr.wowhead.com/item={@id}" class="staticTip itemToolTip">		
<span class="stats_rarity{@rarity}"><xsl:value-of select="@name"/></span></a>
            ...
            <xsl:if test="@gem0Id != '0'">
                <a class="staticTip itemToolTip" id="i={@gem0Id}&amp;{$c_Url}" href="http://fr.wowhead.com/item={@gem0Id}"><img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@gemIcon0}.png"/></a> 
            </xsl:if>
            <xsl:if test="@gem1Id != '0'">
                <a class="staticTip itemToolTip" id="i={@gem1Id}&amp;{$c_Url}" href="http://fr.wowhead.com/item={@gem1Id}"><img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@gemIcon1}.png"/></a> 
            </xsl:if>
            <xsl:if test="@gem2Id != '0'">
                <a class="staticTip itemToolTip" id="i={@gem2Id}&amp;{$c_Url}" href="http://fr.wowhead.com/item={@gem2Id}"><img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@gemIcon2}.png"/></a> 
            </xsl:if>
			 </td>
            <td class="glist_ench">
                <xsl:if test="@permanentenchant != '0'">
                	<a class="staticTip itemToolTip" id="i={@permanentEnchantItemId}&amp;{$c_Url}" href="http://fr.wowhead.com/item={@permanentEnchantItemId}">
                    <xsl:if test="@permanentEnchantSpellDesc">
                     ...
                     <img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@permanentEnchantIcon}.png"/>
                    </a> 
                </xsl:if>
             </td>
        </tr>
        ...
        <xsl:variable name="modelBasePath" select="character/modelBasePath/@value"  />		
		 ...
		 if(!charUrl) var charUrl = '<xsl:value-of select="character/@charUrl"/>'
		  ...
		  {		var lang = "<xsl:value-of select="$lang"/>".split("_")[0].toLowerCase()
                ...
                var embedlink = '&lt;iframe src="'+String(window.location).split('character')[0]+'character-model-embed.php?<xsl:value-of select="character/@charUrl"/>&amp;rhtml=true" scrolling="no" height="588" width="321" frameborder="0"&gt;&lt;/iframe&gt;'
                ...
                var flashvars = { character: theCharName, modelUrl: "character-model.php?"+encodeURIComponent(charUrl), fileServer: "models/", 
...
<xsl:template name="gearSlot">
	...
	<div class="gearItem">
		<xsl:choose>
			<xsl:when test="$slotInfo">
				<xsl:attribute name="style">
					background-image: url('http://eu.wowarmory.com/wow-icons/_images/51x51/<xsl:value-of select="$slotInfo/@icon"/>.jpg')
				</xsl:attribute>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:if test="$slotInfo and not($slotNum = '3' or $slotNum = '18')">
			<div class="fly-horz">
				<a class="upgrd" href="search.php?searchType=items&amp;{$recUrl}&amp;pi={$slotInfo/@id}">
					<xsl:value-of select="$loc/strs/characterSheet/str[@id='armory.character-sheet.findupgrade']" />
				</a>
			</div>
		</xsl:if>
		<a id="i={$slotInfo/@id}&amp;{character/@charUrl}&amp;s={$slotNum}" href="http://fr.wowhead.com/item={$slotInfo/@id}" class="staticTip itemToolTip gearFrame">		
	...
	<xsl:choose>
		<xsl:when test="$teamNode">
			<div class="arenacontainer">
				...
				<div class="icon" onclick="window.location='team-info.php?{$teamNode/@teamUrl}'">
					<xsl:choose>
