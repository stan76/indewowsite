...
<xsl:template name="printGemSocket">
    ...
    <xsl:choose>
        <xsl:when test="string-length($enchant) &gt; 0">
            <br /><xsl:if test="string-length($icon) &gt; 0"><img src="{concat('http://eu.wowarmory.com/wow-icons/_images/21x21/',$icon,'.png')}" class="socketImg p"/></xsl:if><xsl:value-of select="$enchant" />
        </xsl:when>
        <xsl:otherwise>
            <xsl:choose>
