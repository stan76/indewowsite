...
<xsl:template match="item" mode="itemInfoContent2">	
	...
	<blockquote>
		<b class="iitems">
			<h4><a href="item-search.php"><xsl:value-of select="$loc/strs/unsorted/str[@id='armory.labels.items']"/></a></h4>
			<h3><xsl:value-of select="$loc/strs/unsorted/str[@id='armory.labels.item-results']"/></h3>
		</b>
	</blockquote>
	
	<div style="background: url(http://eu.wowarmory.com/wow-icons/_images/51x51/{@icon}.jpg) 0 0 no-repeat; height: 51px; width: 51px; float: left;  margin: 10px;"></div>

	<div style="background-color:#330000; padding: 8px 12px; width: 290px; float: left; margin: 10px 0;">
		<xsl:variable name="itemTooltipNode" select="document(concat('http://fr.wowhead.com/item=',@id))/page/itemTooltips/itemTooltip" />
...
<script type="text/javascript" src="http://static.wowhead.com/widgets/power.js"></script>	
						...
						<div class="item-bottom">
							<div class="item-top">
								...
								<div class="icon-container">
									<p></p>
									<img src="http://eu.wowarmory.com/wow-icons/_images/64x64/{$itemTooltipNode/icon}.jpg" class="p"/>
								</div>
												...
												<strong>
													<xsl:choose>
														<xsl:when test="dropCreatures/creature[1]/@url">
															<a href="search.php?searchType=items&amp;{dropCreatures/creature[1]/@url}"><xsl:value-of select="dropCreatures/creature[1]/@name" /></a>
														</xsl:when>
														<xsl:otherwise>
													...
													<strong>
														<xsl:choose>
															<xsl:when test="containerObjects/object[1]/@url">
																<a href="search.php?searchType=items&amp;{containerObjects/object[1]/@url}"><xsl:value-of select="containerObjects/object[1]/@name" /></a>
															</xsl:when>
															<xsl:otherwise>
													...
													<p>
														...
														<strong>
															<xsl:for-each select="cost/token">
																<a class="item-count staticTip itemToolTip" id="i={@id}" href="http://fr.wowhead.com/item={@id}"><span><b><xsl:value-of select="@count" /></b><xsl:value-of select="@count" /></span> <img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png" class="p21" /></a>
															</xsl:for-each>
														</strong>
													</p>
												</xsl:if>											
														...
														<table cellpadding="0" cellspacing="0" border="0" style="position:relative">
															<tr>
																<td	style="vertical-align:middle;line-height:1em"><xsl:choose>
																	...
																	</xsl:choose><img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png" class="p21s staticTip itemToolTip" id="i={@id}" /></td>
																<td	style="vertical-align:middle; padding-left:7px"><a class="rarity{@quality}" href="http://fr.wowhead.com/item={@id}" id="i={@id}"><xsl:value-of select="@name" /></a></td>
															</tr>
														</table>
													</div>
												</xsl:for-each>
					...
					<xsl:for-each select="containerObjects/object">
						...
						<tr>
							<td>
								<q><span><i>
									<xsl:choose>
										<xsl:when test="@url"><a href="search.php?searchType=items&amp;{@url}"><xsl:value-of select="@name" /></a></xsl:when>
							...
							<td>
								<q>
								<xsl:choose>
									<xsl:when test="@areaUrl"><a href="search.php?searchType=items&amp;{@areaUrl}"><xsl:value-of select="@area" /><xsl:if test="@heroic"><xsl:value-of select="$loc/strs/itemInfo/str[@id='armory.item-info.heroic']"/></xsl:if></a></xsl:when>
	...
	<xsl:if test="createdBy and count(createdBy/spell/reagent) &gt; 0">
		...
		<div class="rel-tab"><p class="rel-reagentreq"></p>
			...
			<img src="http://eu.wowarmory.com/wow-icons/_images/43x43/{createdBy/spell/item/@icon}.png" class="p43" />
			</xsl:when>
			<xsl:otherwise>
			<img src="http://eu.wowarmory.com/wow-icons/_images/43x43/{@icon}.png" class="p43" />
			</xsl:otherwise>
			</xsl:choose>
			</td>
			<td><q>
			<xsl:choose>
			<xsl:when test="createdBy/spell/item">
			<a class="rarity{createdBy/spell/item/@quality}" href="http://fr.wowhead.com/item={createdBy/spell/item/@id}" id="i={createdBy/spell/item/@id}"><xsl:value-of select="createdBy/spell/item/@name" /></a>
			
			</xsl:when>
			<xsl:otherwise>
			<a class="rarity{@quality}" href="http://fr.wowhead.com/item={@id}" id="i={@id}"><xsl:value-of select="@name" /></a>
			...
			<a class="item-add" href="http://fr.wowhead.com/item={@id}" id="i={@id}"><img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png" class="p21" /><xsl:if test="@count &gt; 1"><b><xsl:value-of select="@count" /></b></xsl:if>
			</a>
			</xsl:for-each>
			</q></td></tr></table>
			</div>
	...
	<xsl:if test="plansFor">
		...
		<div class="rel-tab">
			...
			<tr>
			<td width="55"><img id="{plansFor/spell/item/@id}" src="http://eu.wowarmory.com/wow-icons/_images/43x43/{plansFor/spell/item/@icon}.png" class="p43 staticTip itemToolTip" /></td>
			<td class="item-icon" width="50%"><q><a class="rarity{plansFor/spell/item/@quality} staticTip itemToolTip" href="http://fr.wowhead.com/item={plansFor/spell/item/@id}" id="i={plansFor/spell/item/@id}"><xsl:value-of select="plansFor/spell/item/@name" /></a>
			</q>
			</td>
			<td style="white-space:nowrap;"><q>
	...
	<div class="scroll-padding"></div><div class="rel-tab"><p class="rel-reagentfor"></p>
		...
		<tr>
		<td width="55"><img id="{item/@id}" src="http://eu.wowarmory.com/wow-icons/_images/43x43/{item/@icon}.png" class="p43 staticTip itemToolTip" /></td>
		<td class="item-icon" width="50%"><q><a class="rarity{item/@quality}" href="http://fr.wowhead.com/item={item/@id}" id="i={item/@id}"><xsl:value-of select="item/@name" /></a></q>
		</td>
		<td style="white-space:nowrap;"><q>
		<xsl:for-each select="reagent">
		<a class="item-add" href="http://fr.wowhead.com/item={@id}" id="i={@id}"><img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png" class="p21" /><b><xsl:if test="@count &gt; 1"><xsl:value-of select="@count" /></xsl:if></b>
		</a>
		</xsl:for-each>
		</q></td></tr>
		</xsl:for-each>
	...
	<xsl:if test="currencyFor">
		<div class="scroll-padding"></div><div class="rel-tab">
			...
			<xsl:sort select="@dropRate" data-type="number" order="descending" />
			<tr>
			<td width="55"><img id="{@id}" src="http://eu.wowarmory.com/wow-icons/_images/43x43/{@icon}.png" class="p43 staticTip itemToolTip" /></td>
			<td class="item-icon"><q><a class="rarity{@quality}" href="http://fr.wowhead.com/item={@id}" id="i={@id}"><xsl:value-of select="@name" /></a></q>
			</td>
			...
			<a class="item-count" href="http://fr.wowhead.com/item={@id}" id="{@id}"><span><b><xsl:value-of select="@count" /></b><xsl:value-of select="@count" /></span> <img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png" class="p21" /></a>
			</xsl:when>
			<xsl:otherwise>
			<a class="item-add" href="http://fr.wowhead.com/item={@id}" id="i={@id}">
			<img src="http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png" class="p21" /><xsl:if test="@count &gt; 1"><b><xsl:value-of select="@count" /></b></xsl:if>
			</a>
			</xsl:otherwise>
			</xsl:choose>
			</xsl:for-each>
	...
	<xsl:if test="disenchantLoot">
		<div class="scroll-padding"></div><div class="rel-tab"><p class="rel-de"></p>
			...
			<xsl:sort select="@dropRate" data-type="number" order="descending" />
			<tr>
			<td width="55"><img src="http://eu.wowarmory.com/wow-icons/_images/43x43/{@icon}.png" class="p43 staticTip itemToolTip" id="{@id}" /></td>
			<td class="item-icon" width="50%"><q><a class="rarity{@quality}" href="http://fr.wowhead.com/item={@id}" id="i={@id}"><xsl:value-of select="@name" /></a></q></td>
