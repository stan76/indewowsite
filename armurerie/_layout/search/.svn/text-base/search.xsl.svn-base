		...
		<div class="info-pane">
			<div class="generic-content">
				<div class="nrcontent">
					...
					<div class="noresult-footer"><a href="index.php"><xsl:value-of select="$loc/strs/unsorted/str[@id='armory.search.label.search.return']"/></a></div>
				</div>
			</div>
		</div>
	</div>
		...
		<xsl:for-each select="tabs/tab">
			<xsl:if test="@count &gt; 0">
				<div class="tab">
					...
					<a href="search.php?searchType={$searchType}&amp;searchQuery={$searchText}&amp;selectedTab={@type}">
...
<xsl:template name="characterResults">
	...
	<tbody>
		<xsl:for-each select="searchResults/characters/character">
			...
			<tr>
				<td><a href="character-sheet.php?{@url}"><xsl:value-of select="@name" /></a></td>
				...
				<td><a href="guild-info.php?{@guildUrl}"><xsl:value-of select="@guild" /></a></td>
...
<xsl:template name="guildResults">
	...
	<tbody>
		<xsl:for-each select="searchResults/guilds/guild">
			...
			<xsl:variable name="factionTxt" select="$loc/strs/unsorted/str[@id=concat('armory.labels.faction.',current()/@factionId)]" />		
			<tr>
				<td><a href="guild-info.php?{@url}"><xsl:value-of select="@name" /></a></td>
				<td><xsl:value-of select="@realm" /></td>
				<td><xsl:value-of select="@battleGroup" /></td>
				<td class="centNum"><img class="staticTip" onmouseover="setTipText('{$factionTxt}');" src="images/icons/faction/icon-{@factionId}.gif" /></td>
				<td><span style="display: none;"><xsl:value-of select="@relevance" />%</span>
...
<script type="text/javascript" src="http://static.wowhead.com/widgets/power.js"></script>
	...
	<tbody>
		<xsl:for-each select="searchResults/items/item">
			<xsl:sort select="@relevance" />
			<tr>
				<td><a id="{@id}" href="http://fr.wowhead.com/item={@id}" class="rarity{@rarity}" style="background: url('http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png') 0 0 no-repeat; padding: 2px 0 4px 25px;"><xsl:value-of select="@name" /></a></td>
				<td class="leftNum"><xsl:value-of select="filter[@name='itemLevel']/@value" /></td>
				<td>
					<xsl:choose>
						<xsl:when test="current()/filter[@name='source']/@areaName">
							<a href="search.php?source=dungeon&amp;dungeon={current()/filter[@name='source']/@areaKey}&amp;boss=all&amp;difficulty=all&amp;type=all&amp;searchType=items"><xsl:value-of select="current()/filter[@name='source']/@areaName" /></a>
						</xsl:when>
						<xsl:otherwise>
...
<xsl:template name="arenaResults">
	...
	<tbody>
		<xsl:for-each select="searchResults/arenaTeams/arenaTeam">
			...
			<xsl:variable name="factionTxt" select="$loc/strs/unsorted/str[@id=concat('armory.labels.faction.',current()/@factionId)]" />
			<tr>
				<td><a href="team-info.php?{@url}"><xsl:value-of select="@name" /></a></td>								
