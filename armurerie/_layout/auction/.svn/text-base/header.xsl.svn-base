...
<xsl:output method="html" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN"/>
	...
	<!-- User Variables -->
    <xsl:variable name="loginStatus" select="document('../../login-status.php')" />  
    <xsl:variable name="username" select="$loginStatus/page/loginStatus/@username" />
    <xsl:variable name="user" select="document('../../character-select.php?sel=2')" />
    <xsl:variable name="char" select="$user/page/characters/character[@selected=1]" />

    <xsl:variable name="wowAnywhere" select="'../../wow-remote.php'" />
    ...
    <xsl:template name="auctionUser">
        ...
        <xsl:if test="/page/auctionStatus or /page/error and /page/error/@code != '10007'">
      	<div id="auctionUser">
        	...
        	<div id="charAvatar">
            	<a href="/character-sheet.php?{$char/@url}" class="staticTip" onmouseover="setTipText('{$loc/strs/character/str[@id='character-level']} {$char/@level} {$txtRace} {$txtClass}')">
					<xsl:choose>
                ...
                <xsl:choose>
                    <xsl:when test="/page/command/@f = 2">
                        <xsl:if test="$char/@factionId = 0">
                            <a href="auctionhouse/index.php?f=a" onclick="Auction.changeHouse('a'); return false;" onmouseover="setTipText('{$loc/strs/auctionHouse/str[@id='house.switchAlliance']}');" class="staticTip"><img src="images/ahouse/icon-alliance.gif" alt="" /></a>
                        </xsl:if>

                        <xsl:if test="$char/@factionId = 1">
                            <a href="auctionhouse/index.php?f=h" onclick="Auction.changeHouse('h'); return false;" onmouseover="setTipText('{$loc/strs/auctionHouse/str[@id='house.switchHorde']}');" class="staticTip"><img src="images/ahouse/icon-horde.gif" alt="" /></a>
                        </xsl:if>
                    </xsl:when>
                    <xsl:otherwise>
                        <a href="auctionhouse/index.php?f=n" onclick="Auction.changeHouse('n'); return false;" onmouseover="setTipText('{$loc/strs/auctionHouse/str[@id='house.switchNeutral']}');" class="staticTip"><img src="images/ahouse/icon-neutral.gif" alt="" /></a>
                    </xsl:otherwise>
                </xsl:choose>
            </div>
	...
	<script type="text/javascript" src="http://static.wowhead.com/widgets/power.js"></script>
        ...
        <tr id="auction_{@auc}">
            ...
            <td valign="middle">
            	<div class="itemList">
                    <a id="{@id}" href="http://fr.wowhead.com/item={@id}" class="rarity{@qual}" onmouseover="setTipParams('{@seed}', '{@rand}', '{@charges}');"><img src="http://eu.wowarmory.com/wow-icons/_images/43x43/{@icon}.png" alt="" /></a>
                    <xsl:if test="@quan > 1">
                        <div class="itemListStack"><xsl:value-of select="@quan" /></div>
                    </xsl:if>
                </div>
            	<a id="{@id}" href="http://fr.wowhead.com/item={@id}" class="rarity{@qual}" onmouseover="setTipParams('{@seed}', '{@rand}', '{@charges}');">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <a href="character-sheet.php?r={$char/@realm}&amp;cn={@seller}" class="sellerName"><xsl:value-of select="@seller" /></a>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <td class="ac">
                <xsl:choose>
                    ...
                    <xsl:when test="@bidder != ''"><a href="character-sheet.php?r={$char/@realm}&amp;cn={@bidder}"><xsl:value-of select="@bidder" /></a></xsl:when>
                        ...
                        <xsl:for-each select="$inventory/items/invItem">
                            ...
                            <tr id="auction_0_{$item}" onclick="AuctionCreate.chooseItem(i0_{$item}, this);">
                                ...
                                <td colspan="2" style="width: 70%">
                                    ...
                                    <a id="{@id}" href="#" class="rarity{@qual}" onmouseover="setTipParams('{@seed}', '{@rand}', '{@charges}');" style="background: url('http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png') 0 0 no-repeat;" onclick="return false;">
                                        <xsl:call-template name="truncate">
                                            <xsl:with-param name="string" select="@n" />
