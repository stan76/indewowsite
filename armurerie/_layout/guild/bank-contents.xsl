...
<xsl:template name="guildContentListing">
	...
	<div style="width: 819px; height: 519px; background: url('images/guildbanks-frame.jpg') top left no-repeat; margin: 0 auto; position:relative; ">
		<div id="whichTabDisplay" class="filtertitle" style="background: url('http://eu.wowarmory.com/wow-icons/_images/43x43/{guildBank/bags/bag[@viewable='true']/@icon}.png') no-repeat;">
			...
			<xsl:for-each select="guildBank/bags/bag[@viewable='true']">
				<xsl:if test="@name != ''">			
					<a href="javascript:void(0)" class="staticTip bankTabIcon" onclick="toggleBankTab(this,'{@id}','{@name}','{@icon}');" 
						...
						style="background:url('http://eu.wowarmory.com/wow-icons/_images/43x43/{@icon}.png') no-repeat;">
        ...
        <table id="bankContentsTable" class="data-table sortTable" cellpadding="0" cellspacing="0" style="width: 100%">
            ...
            <tbody>
				<xsl:for-each select="guildBank/items/item">				
					<tr>
						<td>							
							<a href="item-info.php?i={@id}" class="staticTip itemToolTip rarity{@qi}" id="i={@id}" 
								style="background: url('http://eu.wowarmory.com/wow-icons/_images/21x21/{@icon}.png') no-repeat 0 50%; padding: 3px 0 3px 25px; line-height: 28px;">
	...
	<xsl:if test="$ctr &lt;= $max">
		...
		<a href="javascript:void(0)" class="singleBankItem {$ctr}" style="cursor: default;">			
			<xsl:if test="$currItem">
				<xsl:attribute name="style">background:url(http://eu.wowarmory.com/wow-icons/_images/43x43/<xsl:value-of select="$currItem/@icon" />.png) no-repeat 4px 1px;</xsl:attribute>
