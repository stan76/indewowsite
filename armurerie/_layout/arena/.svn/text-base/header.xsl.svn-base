			...
			<div class="reldiv">
				...
				<div class="arenareport-moldingleft-name">
					<div class="reldiv">
						...
						<div class="teamnamehighlight"><a class="teamnamehighlight" href="team-info.php?{$docHeader/@url}"><xsl:value-of select="$docHeader/@name"/>
						...
						<div style="position: absolute; top:-1px;">						
							<div class="team-members"><xsl:value-of select="$loc/strs/arena/str[@id='team-members']"/>
								<xsl:for-each select="members/character">
									<a href="character-sheet.php?{@charUrl}"><xsl:value-of select="@name"/></a><xsl:if test="position() != last()">, </xsl:if>
								</xsl:for-each>
							</div>	
						</div>
					</div>
