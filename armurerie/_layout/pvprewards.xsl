	...
	<div class="pvprewards">
		...
		<xsl:for-each select="document('../_data/items/pvpa.xml')/pvpVendors/pvpVendor">
			...
			<tr class="expand-list">
				<td valign="top" class="{$altId}">	 
					<a href = "search.php?source=pvpAlliance&amp;pvp={$typeKey}&amp;type=all&amp;searchType=items"><xsl:value-of select = "$loc/strs/pvpSource/str[@id=concat('armory.pvprewards.', $typeKey)]" /></a>
				</td> 
			</tr>
    </xsl:for-each>
		</table>
		...
		<table>
		 <xsl:for-each select="document('../_data/items/pvph.xml')/pvpVendors/pvpVendor">
			...
			<tr class="expand-list">
				<td valign = "top" class="{$altId}">	 
					<a href = "search.php?source=pvpHorde&amp;pvp={$typeKey}&amp;type=all&amp;searchType=items"><xsl:value-of select = "$loc/strs/pvpSource/str[@id=concat('armory.pvprewards.', $typeKey)]" /></a>
				</td> 
			</tr>
     </xsl:for-each>
		</table>
