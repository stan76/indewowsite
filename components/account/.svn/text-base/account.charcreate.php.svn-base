    ...
    elseif($_GET['action']=='createchar'){
        ...
        ####################### INSERT TO Database ########################
          ...
          if (strstr($COPY_BAG_SUB_item_instance['data'], $COPY_WORK_item_instance['old_guid']) == TRUE){
			...
			'".$COPY_character['power6']."', '".$COPY_character['power7']."', '".$COPY_character['specCount']."', '".$COPY_character['activeSpec']."\')");
        ...
        // update points
        $query = $DB->query("UPDATE `voting_points` SET `points`=(`points` - ".$char_points.") WHERE id=?d",$account_id);
