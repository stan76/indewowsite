<br>
<?php builddiv_start(1, "Ajouter un royaume") ?>
...
<?php if(empty($_GET['action'])){ ?>
    ...
    <form action="index.php?n=admin&sub=realms&action=create" method="post" onSubmit="return popup_ask('<?php echo $lang['sure_q'];?>');">
		...
		<tr>
            ...
            <td width="45" align="center" colspan="2">DB Info: <input type="text" name="dbinfo" value="<?php echo isset($item['dbinfo']) ? $item['dbinfo'] : ''; ?>" style="width:100%;font-size:0.7em;"></td>
        </tr>
        <tr>
...
<?php foreach($items as $item){ ?>
        <tr class="normal">
			...
			<td align="center">****</td>
            <td>Not viewable(For security)</td>
        </tr>
<?php } ?>
    </tbody>
...
<script language="javascript">
    ...
    <form action="index.php?n=admin&sub=realms&action=update&id=<?php echo $item['id']; ?>" method="post" onSubmit="return confirm('<?php echo $lang['sure_q'];?>');">
        <tr>
            <td align="center" colspan="1"><b><?php echo $item['id']; ?></b></td>
            <td colspan="2" ><input type="text" name="name" value="<?php echo $item['name']; ?>" style="width:100%;font-size:0.7em;"></td>
            <td colspan="2"><input type="text" name="address" value="<?php echo $item['address']; ?>" style="width:100%;font-size:0.7em;"></td>
            <td colspan="2" align="center"><input type="text" name="port" value="<?php echo $item['port']; ?>" style="width:100%;font-size:0.7em;"></td>
            <td colspan="2" align="center"><select style="width:100%;font-size:0.7em;" name="icon"><?php foreach($realm_type_def as $tmp_id=>$tmp_name){ echo'<option value="'.$tmp_id.'" '.($item['icon']==$tmp_id?'selected':'').'>'.$tmp_name.'</option>'; } ?></select></td>
            <td colspan="2"><select style="width:100%;font-size:0.7em;" name="timezone"><?php foreach($realm_timezone_def as $tmp_id=>$tmp_name){ echo'<option value="'.$tmp_id.'" '.($item['timezone']==$tmp_id?'selected':'').'>'.$tmp_name.'</option>'; } ?></select></td>
		</tr>	
			<td colspan="2" align="center"><input type="text" name="ra_port" value="<?php echo $item['ra_address']; ?>" style="width:100%;font-size:0.7em;"></td>
			<td colspan="2" align="center"><input type="text" name="ra_port" value="<?php echo $item['ra_port']; ?>" style="width:100%;font-size:0.7em;"></td>
			<td colspan="2" align="center"><input type="text" name="ra_user" value="<?php echo $item['ra_user']; ?>" style="width:100%;font-size:0.7em;"></td>
			<td colspan="2" align="center"><input type="password" name="ra_pass" value="<?php echo $item['ra_pass']; ?>" style="width:100%;font-size:0.7em;"></td>
            <td colspan="3" align="center"><input type="text" name="dbinfo" value="Not viewable" style="width:100%;font-size:0.7em;"></td>
        </tr>
        <tr>
            <td colspan="11" style="background:#a7a7a7;height:8px;border:2px solid #000;">
