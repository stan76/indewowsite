...
var site_link = '<?php echo "/index.php" ?>';
var forum_link = '<?php echo "/index.php?n=forum" ?>';
var armory_link = '<?php echo "/armory/index.php" ?>';</script>
...
<?php
$accountid = $user['id'];
$resulttest = mysql_query("SELECT count(*) FROM realmd.pms WHERE owner_id=$accountid AND showed=0");
if($user['id']>0):
    $userpm_num = $auth->check_pm();
?>
                      <input type="hidden" name="action" value="logout"/>
                      <?php echo empty($user['username']) ? $user['username'] : $user['username'];?>
...
<?php else: ?>
                      ...
                      MDP: <input name="pass" size="14" type="password"/>
